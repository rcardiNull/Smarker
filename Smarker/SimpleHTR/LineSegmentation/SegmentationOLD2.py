...
def preprocessing():
    ...
    return thresh_img,sorted_contour_lines,dilated
...
def textSegmentation():
    ...
    # dilation
    thresh_img,sorted_contour_lines,dilated=preprocessing()
    ...
    img4 = img.copy()
    n = 0
    images_path = "/home/pi2/camCombo/Smarker/SimpleHTR/segOut"
    for n in range(len(line_list[:])):
        ...
        entry.save(f"{images_path}/entry_{b}.png")
        entry_name=f"entry_{b}.png"
        print(f"File: {entry_name}")
        print(f"Saving word to :  {images_path}")


# *************** MainLoop *********************************
# Working spot: need to loop over all files that get transferred from Zero in case there are multiple takes. <---------------
sourceDir='/home/pi2/camCombo/Smarker/SimpleHTR/LineSegmentation/sourceImg/'
for filename in os.scandir(sourceDir):
    if filename.is_file():
        print("Found a file with name: ",filename)
        fname=os.path.basename(filename)
        print("Loading imgage: "+str(fname))
        img = cv2.imread("/home/pi2/camCombo/Smarker/SimpleHTR/LineSegmentation/sourceImg/"+str(fname))  # input image file.
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        h, w, c = img.shape  # load dimensions
        print("Resizing image: "+str(fname))
        # resize the image
        if w > 1000:
            new_w = 1000
            ar = w/h  # aspect ratio
        ...
        print("Starting Segmentation on image: "+str(fname))
        thresholding(img)
        preprocessing()
        textSegmentation()
